...
async function sendLastTransactionDetails(fromNumber) {
    ...
    try {
        ...
        if (!user || !user.last_transaction) {
            twiml.message("We couldn't find any transaction history for your account.");
        } else {
            // Format the last transaction details
            const transactionDate = user.last_transaction.toLocaleDateString('en-US', {
                weekday: 'short', year: 'numeric', month: 'short', day: 'numeric',
            });
            const transactionTime = user.last_transaction.toLocaleTimeString('en-US', {
...
`;
        ...
        twiml.message('An error occurred while fetching your transaction details.');
    }
